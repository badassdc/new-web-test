...
const Giveaways = ({ $, title, description, giveaways, id, type }) => {
  ...
  const reroll = async id => {
    const _reroll = await axios.get('https://custom-giveaways.ml/v1/giveaway/'+id+'/reroll?_token='+window?.localStorage?.getItem("$Award_token")).then(res => res.data);
    if(_reroll) {
      if(_reroll.success) {
  ...
  const deletegiveaway = async id => {
    const _delete = await axios.get('https://custom-giveaways.ml/v1/giveaway/'+id+'/cancel?_token='+window?.localStorage?.getItem("$Award_token")).then(res => res.data);
    if(_delete) {
      if(_delete.success) {
