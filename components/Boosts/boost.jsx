...
export default function Boost({ isOpen, closeModal, user, guild, boost, mutate }) {
  ...
  const boostServer = async id => {
    ...
    const _boost = await axios.put(
        "https://custom-giveaways.ml/v1/boost/" + guild.id +
    ...
    <Transition appear show={isOpen} as={Fragment}>
        ...
        >
            ...
            >
                ...
                <div className="my-5">
                    <div className={"grid grid-cols-3 bg-black/20 text-white rounded-xl " + ((boost.user.boosts || []).length > 6 ? "h-96 overflow-y-scroll" : "")}>
                        <div className="col-span-2">
                            {(boost.user.boosts || []).filter(_b => !_b.used_in && Date.now() < (_b.expires_at || Infinity)).concat((boost.user.boosts || []).filter(_b => _b.used_in && Date.now() < (_b.expires_at || Infinity))).concat((boost.user.boosts || []).filter(_b => Date.now() >= (_b.expires_at || Infinity))).map((_boost, __i) => (
                                <div key={__i} className={"h-16 space-x-2 border-b border-white/10 flex items-center justify-center " + (__i % 2 == 1 ? "bg-black/10" : "")}>
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            {Date.now() >= (_boost.expires_at || Infinity) ? <span className="text-rose-400 bg-rose-400 bg-opacity-10 py-0.5 px-2 rounded-xl text-xs">
                                                Expired
                                            </span> : (
                                                _boost.used_in ? <span className="text-sky-400 bg-sky-400 bg-opacity-10 py-0.5 px-2 rounded-xl text-xs">
                                                    Using at <b className="font-semibold">{_boost.used_in}</b>
                                                </span> : <span className="text-emerald-400 bg-emerald-400 bg-opacity-10 py-0.5 px-2 rounded-xl text-xs">
                                                    Available
                                                </span>
                                            )}
                                        </div>
                        ...
                        <div>
                            {(boost.user.boosts || []).filter(_b => !_b.used_in && Date.now() < (_b.expires_at || Infinity)).concat((boost.user.boosts || []).filter(_b => _b.used_in && Date.now() < (_b.expires_at || Infinity))).concat((boost.user.boosts || []).filter(_b => Date.now() >= (_b.expires_at || Infinity))).map((_boost, __i) => (
                                <div key={__i} className={"h-16 space-x-2 border-b border-white/10 flex items-center justify-end px-3 " + (__i % 2 == 1 ? "bg-black/10" : "")}>
                                    <button onClick={() => boostServer(_boost.id)} className={"flex items-center justify-center font-semibold w-28 py-2 rounded-xl shadow-lg " + (Date.now() >= (_boost.expires_at || Infinity) ? "opacity-50 cursor-not-allowed text-rose-400 bg-rose-400/10 shadow-rose-600/5" : (_boost.used_in ? "opacity-50 cursor-not-allowed text-sky-400 bg-sky-400/10 shadow-sky-600/5" : "hover:opacity-75 cursor-pointer text-emerald-400 bg-emerald-400/10 shadow-emerald-600/5"))}>
                                        {loading == _boost.id ? <i className="py-1 fad fa-spinner-third fa-spin" /> : (Date.now() >= (_boost.expires_at || Infinity) ? "Expired" : (_boost.used_in ? "Using" : "Select"))}
                                    </button>
                                </div>
                            ))}
                        </div>
