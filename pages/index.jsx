...
export default function Index({ $token, $url, $ }) {
  ...
  if (typeof window !== "undefined") {
    window.addEventListener("Custom Giveaways", (event) => {
	...
	const addBot = () => {
		let $w = window.open(
			"https://custom-giveaways.ml/v1/invite/bot?__w=1&__beta=" + 
		...
		let $i = setInterval(async () => {
			...
			const check = await axios.get("https://custom-giveaways.ml/v1/auth/me?_token=" + window?.localStorage?.getItem("$Award_token"));
			clearInterval($i);
		}, 1000);
	};
  const { data: _user } = swr("https://custom-giveaways.ml/v1/auth/me");
  const user = _user ? _user.data : null;
  const { data: _stats } = swr("https://custom-giveaways.ml/v1/others/stats");
      ...
      <div className="py-20 mb-40 max-w-3xl mx-auto">
        ...
        <div className="animateHeader mt-10 flex flex-wrap items-center justify-center gap-x-4">
          ...
          <a
            ...
            href="https://custom-giveaways.ml/v1/invite/discord"
            target="_blank"
          >
