...
const Giveaways = ({ $ }) => {
    ...
    const { data: _guild } = swr("https://custom-giveaways.ml/v1/guilds/" + id + "/check");
    const guild = _guild ? _guild.data : null;

    const { data: _stats } = swr("https://custom-giveaways.ml/v1/guilds/" + id + "/stats");
    const stats = _stats ? _stats.data : null;
    
    const { data: _giveaways } = swr("https://custom-giveaways.ml/v1/giveaway/list?id=" + id);
            ...
            {!_user || !_guild || !stats ? <div className="flex col-span-6 items-center justify-center">
                ...
                <div className="col-span-5 pt-5 lg:pt-0 lg:pl-20 w-full">
                    <TypeFilter 
                        $={$}
                        title="All Giveaways" 
                        description="Here, your giveaways are listed."
                        giveaways={giveaways}
                        id={id}
                        type="ALL"
                    />
